...
window.onload = () => {
    ...
    for (let i = 1; i <= 6; i++) {
        ['A', 'B', 'C', 'D'].forEach(c => {
            classSelect.innerHTML += `<option value="${i}${c}">${i}${c}</option>`;
        });
    }
};
...
function endGame(result) { clearInterval(timer); document.getElementById('screen-single').classList.remove('combo-aura'); if (result === "win") { epicFireworks(); const bonus = hp * 2; score += bonus; Swal.fire({ title: '🎉 恭喜破關！', html: `成功擊敗病魔！<br>血量獎勵：+${bonus}分<br><b style="font-size:2rem; color:#FF5722;">最終總分：${score}</b>`, icon: 'success', confirmButtonText: '查看榮譽榜', confirmButtonColor: '#9C27B0', backdrop: `rgba(0,0,123,0.4)` }).then(() => { saveRecordAndGo('screen-leaderboard'); loadLeaderboard(); }); } else { Swal.fire({ title: '💀 挑戰失敗...', html: `你被病魔打敗了<br><b style="font-size:1.5rem; color:#333;">最終總分：${score}</b><br>去溫習專區看看再來挑戰吧！`, icon: 'error', confirmButtonText: '返回主選單' }).then(() => { saveRecordAndGo('screen-menu'); }); } }
...
function startMultiPlayer() {
    ...
    Swal.fire({ title: '⚔️ 雙人對戰', text: '請將 iPad 平放於桌面，兩人各佔一邊！', icon: 'info', timer: 2000, showConfirmButton: false })
    .then(() => nextMultiQuestion());
}

function updateMultiStats() {
    document.getElementById('p1-hp').innerText = p1Hp;
    document.getElementById('p2-hp').innerText = p2Hp;
}
...
function nextMultiQuestion() {
    ...
    // 洗牌一次，確保兩邊看到的選項位置一模一樣
    let ops = q.options.map((opt, idx) => ({ text: opt, isCorrect: idx === 0 }));
    for (let i = ops.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [ops[i], ops[j]] = [ops[j], ops[i]]; }

    const p1Area = document.getElementById('p1-options'); p1Area.innerHTML = "";
    const p2Area = document.getElementById('p2-options'); p2Area.innerHTML = "";

    ops.forEach((obj) => {
        // P1 按鈕
        const btn1 = document.createElement('button');
        btn1.className = 'btn'; btn1.style.background = '#fff'; btn1.style.color = '#333'; btn1.style.border = '2px solid #ccc';
        btn1.innerText = obj.text;
        btn1.onclick = () => handleMultiClick(1, obj.isCorrect, btn1);
        p1Area.appendChild(btn1);

        // P2 按鈕
        const btn2 = document.createElement('button');
        btn2.className = 'btn'; btn2.style.background = '#fff'; btn2.style.color = '#333'; btn2.style.border = '2px solid #ccc';
        btn2.innerText = obj.text;
        btn2.onclick = () => handleMultiClick(2, obj.isCorrect, btn2);
        p2Area.appendChild(btn2);
    });

    multiTimeLeft = 10;
    document.getElementById('multi-timer').innerText = multiTimeLeft;
    
    multiTimer = setInterval(() => {
        multiTimeLeft--; document.getElementById('multi-timer').innerText = multiTimeLeft;
        if (multiTimeLeft <= 0) {
            clearInterval(multiTimer);
            p1Hp -= 50; p2Hp -= 50; updateMultiStats();
            flashMultiDamage(1); flashMultiDamage(2);
            document.getElementById('p1-question').innerHTML = "<span style='color:red;'>超時！雙方各扣 50 血！</span>";
            document.getElementById('p2-question').innerHTML = "<span style='color:red;'>超時！雙方各扣 50 血！</span>";
            setTimeout(nextMultiQuestion, 1500);
        }
    }, 1000);
}

function handleMultiClick(playerNum, isCorrect, btnElement) {
    if (playerNum === 1 && p1Locked) return;
    if (playerNum === 2 && p2Locked) return;

    btnElement.style.background = isCorrect ? '#4CAF50' : '#f44336';
    btnElement.style.color = 'white';

    if (isCorrect) {
        clearInterval(multiTimer); // 有人答對就停止這題
        p1Locked = true; p2Locked = true; // 鎖定雙方按鈕
        
        if (playerNum === 1) {
            p2Hp -= 100; flashMultiDamage(2);
            showFloatingText('-100', '#d32f2f', 'p2-zone');
            document.getElementById('p1-question').innerHTML = "<span style='color:green;'>✔️ 搶答成功！給予對手 100 傷害！</span>";
            document.getElementById('p2-question').innerHTML = "<span style='color:red;'>❌ 對手搶答成功！你被扣血！</span>";
        } else {
            p1Hp -= 100; flashMultiDamage(1);
            showFloatingText('-100', '#d32f2f', 'p1-zone');
            document.getElementById('p2-question').innerHTML = "<span style='color:green;'>✔️ 搶答成功！給予對手 100 傷害！</span>";
            document.getElementById('p1-question').innerHTML = "<span style='color:red;'>❌ 對手搶答成功！你被扣血！</span>";
        }
        updateMultiStats();
        setTimeout(nextMultiQuestion, 1500);
    } else {
        // 答錯，自己扣 50，且被鎖定，對手可以繼續猜
        if (playerNum === 1) {
            p1Hp -= 50; p1Locked = true; flashMultiDamage(1);
            showFloatingText('-50', '#1976D2', 'p1-zone');
            document.getElementById('p1-question').innerText = "❌ 答錯扣 50 滴血！(等待對手作答)";
        } else {
            p2Hp -= 50; p2Locked = true; flashMultiDamage(2);
            showFloatingText('-50', '#1976D2', 'p2-zone');
            document.getElementById('p2-question').innerText = "❌ 答錯扣 50 滴血！(等待對手作答)";
        }
        updateMultiStats();
        ...
        if (p1Locked && p2Locked) {
            clearInterval(multiTimer);
            document.getElementById('p1-question').innerHTML = "<span style='color:red;'>雙方都答錯了！換下一題！</span>";
            document.getElementById('p2-question').innerHTML = "<span style='color:red;'>雙方都答錯了！換下一題！</span>";
            setTimeout(nextMultiQuestion, 1500);
        }
    }
}

function flashMultiDamage(playerNum) {
    const zone = document.getElementById(playerNum === 1 ? 'p1-zone' : 'p2-zone');
    zone.classList.add('damage-flash');
    zone.classList.add('shake-anim');
    setTimeout(() => {
        zone.classList.remove('damage-flash');
        zone.classList.remove('shake-anim');
    }, 400);
}

function endMultiGame() {
    clearInterval(multiTimer);
    epicFireworks(); // 不管誰贏都放煙火
    let winnerText = "";
    if (p1Hp <= 0 && p2Hp <= 0) winnerText = "平手！同歸於盡！";
    else if (p2Hp <= 0) winnerText = "👑 玩家 1 (上方) 獲勝！";
    else winnerText = "👑 玩家 2 (下方) 獲勝！";

    Swal.fire({
        title: '戰鬥結束', html: `<b style="font-size:2rem; color:#FF5722;">${winnerText}</b>`,
        icon: 'success', confirmButtonText: '返回主選單', backdrop: `rgba(0,0,123,0.4)`
    }).then(() => showScreen('screen-menu'));
}
...
function loadLeaderboard() { document.getElementById('lb-class-name').innerText = currentUser.class; document.getElementById('class-leaderboard').innerHTML = "<p style='text-align:center;'>載入中...</p>"; document.getElementById('grade-leaderboard').innerHTML = "<p style='text-align:center;'>載入中...</p>"; db.ref('records').once('value').then(snapshot => { const data = snapshot.val(); if (!data) { document.getElementById('class-leaderboard').innerHTML = "<p style='text-align:center;'>尚無紀錄</p>"; document.getElementById('grade-leaderboard').innerHTML = "<p style='text-align:center;'>尚無紀錄</p>"; return; } const highestScores = {}; for (let key in data) { const r = data[key]; const studentId = `${r.class}_${r.number}`; if (!highestScores[studentId] || r.score > highestScores[studentId].score) { highestScores[studentId] = r; } } const allStudents = Object.values(highestScores).sort((a, b) => b.score - a.score); const classStudents = allStudents.filter(s => s.class === currentUser.class).slice(0, 10); const gradeStudents = allStudents.slice(0, 20); const renderList = (arr, elementId) => { const container = document.getElementById(elementId); container.innerHTML = ""; arr.forEach((s, idx) => { let icon = idx === 0 ? '🥇' : idx === 1 ? '🥈' : idx === 2 ? '🥉' : `<span style="display:inline-block;width:24px;text-align:center;">${idx+1}</span>`; let topClass = idx <= 2 ? `top-${idx+1}` : ''; let isMeClass = (s.class === currentUser.class && s.number === currentUser.number) ? 'is-me' : ''; container.innerHTML += `<div class="rank-item ${topClass} ${isMeClass}"><div>${icon} &nbsp; <b>${s.class}</b>班 <b>${s.number}</b>號 ${isMeClass ? '(你)' : ''}</div><div style="font-weight:900; font-size:1.3rem;">${s.score} <span style="font-size:0.8rem; font-weight:normal;">分</span></div></div>`; }); }; renderList(classStudents, 'class-leaderboard'); renderList(gradeStudents, 'grade-leaderboard'); }); }
